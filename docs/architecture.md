...
│       ├── Mcp/                        # MCP server: 13 [McpServerToolType] classes = 57 tools (mapped at /mcp, ApiKey scheme)
...
| HmoneyComprehensiveDataProvider | Comprehensive stock data from 24hmoney (financials, reports, dividends, foreign trading, recommendations). **Hợp đồng bắt lại 2026-08-11** — xem "Lệch hợp đồng 24hmoney" bên dưới | HttpClient, IMemoryCache |
...
| Risk | `/api/v1/risk` | Summary, drawdown, VaR, correlation, stop-loss targets, **stress-test (P2)**, **budget (P4)**, **sector-exposure (ADR-0012)**, **volatility-sizing (ADR-0014)** |
...
|----------|---------|---------|-----------|
| 24hmoney | `api-finance-t19.24hmoney.vn` | Real-time prices, history, company list, comprehensive stock data | 15s prices, 30min companies |
...
**Trần khối lượng theo ngân sách biến động (ADR-0014, 2026-08-12):** ràng buộc đầu tiên nhìn vào **quan hệ** giữa mã sắp mua và vị thế đang giữ, không chỉ quy mô từng mã.

- `GET /api/v1/risk/portfolio/{portfolioId}/volatility-sizing?symbol=&entryPrice=&quantity=` → `VolatilitySizingResult`. Cả ba tham số **bắt buộc** và đều nullable ở chữ ký: kiểu không-nullable bind giá trị thiếu thành 0 trong im lặng, mà `quantity = 0` cho "biến động sau lệnh" bằng đúng biến động hiện tại.
- MCP tool `get_volatility_sizing` (ReadOnly) — agent lập được kế hoạch qua `TradePlanTools` nên phải đọc được cùng lan can, nếu không luật chỉ tồn tại trên đường người bấm.
- `VolatilityBudgetCalculator` (`Application/Common/`) — tĩnh, thuần, không I/O. Toàn bộ toán, kiểm thử bằng mảng số không cần Moq.
- **Chỉ nhân, không nghịch đảo Σ.** σ_p = √(wᵀΣw) là phép nhân; toàn bộ tai tiếng "error maximizer" của Markowitz nằm ở Σ⁻¹. Với trần cứng 65 quan sát, Σ đủ ổn định để nhân nhưng quá mỏng để nghịch đảo. Danh mục hiện tại được gộp thành **một tài sản tổng hợp** (chuỗi lợi suất có trọng số), nên bài toán trần rút về hai tài sản và có nghiệm đóng.
- **Bốn bất biến dễ phá:**
  1. Mọi chuỗi lợi suất mang theo **ngày** và ghép theo ngày (`AlignByDate`), không theo vị trí. Ghép theo đuôi cùng độ dài là sai ngay ở ca thường gặp nhất — chính bộ lọc lợi suất bất thường bỏ quan sát ở GIỮA chuỗi, nên mọi cặp trước điểm đó lệch một phiên.
  2. `ObservationCount` là số cặp ghép được thật (`AlignedObservationCount`), không phải `Math.Min` hai độ dài — hai chuỗi cùng số phiên vẫn có thể lệch tập ngày.
  3. `MarginalRiskContributionPercent` nhận hiệp phương sai và phương sai **cùng đơn vị lợi suất phiên bình phương**, không nhận con số %/năm.
  4. `MaxQuantityWithinBudget = null` + `IsUnconstrainedByVolatility = true` nghĩa là *không bị ràng buộc*; `null` + `false` nghĩa là *không tính được*. Gộp hai ca là biến "không biết" thành "thoải mái".
- **Lấy vị thế qua `PositionBuilder`** (ADR-0010), không qua `GetPortfolioRiskSummaryAsync` — đường đó tra beta từng mã, mỗi lần 9 request HTTP, không dùng được ở nhịp debounce 500ms. Trọng số dùng giá đóng cửa cuối của chính chuỗi vừa lấy, không thêm lần gọi giá nào.
- **`IMarketDataProvider.GetDailyHistoryAsync`** là đường riêng gọi thẳng `type=3`. Ánh xạ `days → type` của `GetHistoricalPricesAsync` sai một bậc (yêu cầu 90 ngày nhận về thanh 3 ngày); **không sửa** trong phạm vi này vì nó phục vụ biểu đồ hiển thị — nợ kỹ thuật ghi riêng trong ADR-0014.
- **`create_trade_plan` (MCP) tự gọi truy vấn trần và nối cảnh báo vào chuỗi trả về** (`McpVolatilityNotice`). Lý do: trên form web panel tự hiện nên không tránh được, nhưng qua agent thì `get_volatility_sizing` là tool RIÊNG và lời dặn "gọi trước khi create_trade_plan" lại nằm trên chính tool đó — agent đi thẳng vào tạo kế hoạch sẽ không thấy gì. Lời dặn không phải cơ chế. Vẫn **không chặn** (ADR-0014 Đ4): kế hoạch tạo bình thường, cảnh báo chỉ đi kèm. Chỉ chạy cho lệnh **Buy có gắn danh mục**; truy vấn hỏng thì nuốt ngoại lệ và trả "chưa kiểm được" — ném ở đó khiến agent tưởng tạo thất bại rồi tạo lại, sinh kế hoạch trùng. Có `CreateTradePlanVolatilityWiringTests` ghim việc đấu nối tách khỏi test câu chữ.
- **`GetDailyHistoryAsync` KHÔNG bọc `try/catch` nuốt lỗi**, khác 8 hàm còn lại trong cùng file. Ngoại lệ được để nổi lên `VolatilityBudgetService.BackfillAsync` — nơi duy nhất gọi nó — và nơi đó phân biệt hai ca: nguồn hỏng ⇒ `FetchFailedSymbols`, nguồn trả rỗng đúng cấu trúc ⇒ `MissingSymbols`. Gộp hai ca là panel nói "FPT chưa đủ lịch sử giá" trong khi FPT có thừa, và người dùng kết luận sai rằng mã đó mới hoặc thanh khoản kém. Cùng cơ chế đã khiến lệch hợp đồng sống nhiều tháng ở `HmoneyComprehensiveDataProvider` (PR #158) và đang được xử lý diện rộng bởi `docs/plans/p1-provider-fail-loudly.md` — lớp này nằm ngoài phạm vi plan đó nên xử lý tại chỗ.
- **`SolveMaxAllocation` tính biệt thức bằng `double`, không phải `decimal`.** `b` tỷ lệ với giá trị danh mục nên `b²` tỷ lệ với bình phương của nó: danh mục 10 tỷ gặp một mã trần/sàn liên tục đã vượt `decimal.MaxValue` (7,9×10²⁸) và ném `OverflowException` — tức panel "cảnh báo, không bao giờ chặn" lại trả 500. Bài toán thuần nhất bậc một theo giá trị danh mục nên có test ghim tỷ lệ (nhân đôi vốn ⇒ nhân đôi trần) để việc dùng `double` không lặng lẽ làm hỏng độ chính xác.
- FE: `RiskService.getVolatilitySizingForPlan`, khóa thứ ba trong **đúng** `forkJoin` kiểm-trước của `trade-plan.component.ts` (cùng debounce 500ms với `gate-status` và `sector-exposure`). Chỉ lệnh MUA, cùng lý do ADR-0012.
...
- `frontend/src/app/features/dashboard/widgets/decision-queue.component.ts` — thêm inline action UI:
  - `🔪 BÁN THEO KẾ HOẠCH` button: chỉ hiện khi `item.tradePlanId` non-empty (StopLossHit từ DTO không carry tradePlanId, dùng "Xử lý →" link điều hướng `/risk-dashboard` thay). **Điều hướng sang `/trades/create` với query params `symbol`/`portfolioId`/`direction=Sell`/`planId`/`price`/`quantity` — KHÔNG resolve và KHÔNG tạo lệnh lúc bấm** (đổi 2026-08-13; xem `docs/project-context.md` về cờ dập cảnh báo không hoàn nguyên). `quantity` lấy từ `TradePlanService.getById`; lấy fail thì vẫn điều hướng, chỉ thiếu ô số lượng. Cùng khuôn params mà trang Vị thế đã dùng cho nút "Ghi nhận bán". Optimistic remove chỉ còn ở đường GIỮ.
...
**Chia sẻ hồ sơ với tài khoản khác (2026-08-11):** thuần clipboard, **không có đường server nào** — mục §1 của `multi-user-access-plan.md` (chia sẻ nhiều người dùng) vẫn chưa làm.

- `buildSharePayload()` dùng lại đúng shape trên, thêm `sharedBy`/`sharedAt`. Khác `buildAiPrompt` ở chỗ **không** kèm schema, **không** kèm câu yêu cầu soát lại, và **không** kèm số liệu 24hmoney — người nhận là người, và máy bên kia tự lấy số liệu được.
- `maskSharerName()` che tên đăng nhập của email, giữ `@domain` (`minh.tran@gmail.com` → `min***@gmail.com`). Giữ domain vì giữa người quen thế là đủ nhận ra ai mà không đủ để gửi thư tới. Chuỗi không có `@` trả nguyên văn.
- Ô "Ghi tên bạn là" điền sẵn từ **`User.email`**, không phải `User.name`: `auth.service.ts` đặt `name = payload.name || payload.email`, nên với tài khoản Google có tên thật thì `name` là **họ tên đầy đủ** — mà che một chuỗi không có `@` sẽ trả nguyên văn, tức là mặc định lộ họ tên thật. Giá trị người dùng sửa được nhớ ở `localStorage.dossierSharerName` và thắng giá trị che mặc định.
- `prependSourceLine()` chèn `Nhận từ {tên} ngày {DD/MM/YYYY}.` lên **đầu** ô Ghi chú khi payload có `sharedBy`. Hồ sơ không lưu trường nguồn nào nên đây là **vệt duy nhất** còn lại sau khi dán — phải nằm ở chỗ người ký chắc chắn đọc phải. Xoá được, vì đó là ghi chú của người dùng. Dán lại đúng nội dung đó không đẻ dòng thứ hai.
- `sharedBy` rỗng ⇒ **bỏ hẳn khoá** khỏi JSON, không gửi chuỗi rỗng: khoá vắng thì rõ nghĩa, chuỗi rỗng bắt bên nhận tự đoán.
...
**`McpErrorTranslator` (`src/InvestmentApp.Api/Mcp/McpErrorTranslator.cs`) — filter cấp server, phủ MỌI tool.** Qua MCP, exception bị che thành `"An error occurred invoking '<tên tool>'."`; chỉ `McpException` giữ được nguyên văn (`ExceptionMiddleware` chỉ phục vụ đường REST). Đăng ký bằng `AddMcpServer(McpErrorTranslator.Configure)` → `Filters.Request.CallToolFilters`. **Điểm móc phải là filter, không phải bọc thân từng tool**: giá trị enum sai vỡ ở bước SDK marshal tham số (`AIFunctionFactory.GetParameterMarshaller`), tức trước khi thân tool chạy — helper bọc-thân không nằm trên đường đi của đúng ca lỗi cần đỡ nhất. Với `JsonException`, translator đọc tên enum từ message của STJ rồi nối tập giá trị hợp lệ vào, để agent tự sửa trong một lượt. `McpErrorTranslator.RunAsync` vẫn giữ cho lỗi phát sinh trong thân tool. Guard: `McpErrorSurfaceTests` + `McpBadEnumValueEndToEndTests` (test riêng khẳng định filter **được cắm vào** — filter đúng mà không đăng ký thì vô nghĩa, và test đi qua filter thủ công không phát hiện ra).

**`McpDossierGate` (`src/InvestmentApp.Api/Mcp/McpDossierGate.cs`) — chỉ còn lo phần mô tả.** `Describe` dịch `DossierGateException` thành câu chỉ dẫn theo từng `reason`, nhắc rõ **agent không ký được**; việc bắt và dịch sang `McpException` do `McpErrorTranslator` lo.

**`move_stop_loss` (`TradePlanTools.cs`) — cửa duy nhất để agent dời SL của kế hoạch đã khớp (ADR-0017).** `update_trade_plan` đi qua `TradePlan.Update()`, vốn throw với plan `Executed`/`Reviewed`, nên trước khi có tool này agent bị chặn đúng ở những kế hoạch đang giữ vị thế — tường chỉ còn dựng cho agent sau khi giao diện đã mở. Tool dispatch sang `UpdateStopLossCommand` → `TradePlan.UpdateStopLossWithHistory`, nơi **gate "nới SL bắt buộc lý do" thực sự nằm** (chiều nới đọc từ `plan.Direction` ở server; gate trong modal frontend chỉ là lớp UX). `reason` đặt sau `ct` với `= null` nên không lọt vào `required` — bắt buộc hay không do entity quyết theo chiều dời, không phải schema. Entity cũng chặn plan `Reviewed`/`Cancelled`: giao diện chỉ hiện nút với `InProgress`/`Executed` nên trước khi có tool này điều kiện đó chưa cần ở tầng dưới, nhưng agent không đi qua giao diện. Mỗi lần nới bị `DisciplineScoreCalculator` đếm, nên `[Description]` của tool tự nói ra điều đó (ADR-0015: hướng dẫn nằm trong schema).

**`hold_decision` (`DecisionTools.cs`) — agent xử lý được Hàng đợi quyết định, nhưng chỉ theo hướng GIỮ.** Dispatch `ResolveDecisionCommand` với `Action` **cố định** `HoldWithJournal`; `action` **không** là tham số của tool, nên `ExecuteSell` không tới được từ MCP. Cố ý: `ExecuteSell` tạo lệnh SELL với khối lượng lấy cứng từ kế hoạch — đường đó vừa bị bỏ khỏi Decision Queue vì không cho sửa số lượng, mở lại cho agent là dựng lại đúng cái vừa tháo. Agent muốn bán thì dùng `create_trade` sau khi thống nhất khối lượng với người dùng. Luật `Note` ≥ 20 ký tự nằm ở `ResolveDecisionCommandValidator` nên tool đi qua đúng cổng đó. `portfolioId`/`tradePlanId` phải truyền lại từ `get_decision_queue`: thiếu thì cờ dập rơi về phạm vi (mã, loại) và giấu luôn cảnh báo cùng mã ở danh mục khác. Guard: `McpToolArgumentBindingTests` khẳng định `Action` luôn là `HoldWithJournal` và `PortfolioId` tới được command.

**`JournalEntryType.Decision` không đặt được từ đường tạo/sửa nhật ký (`JournalEntryTypeRule`).** Loại này là **cờ dập cảnh báo** — `GetDecisionQueueQuery.LoadResolvedTodayAsync` gom mọi mục `Decision` trong ngày VN rồi lọc thẻ tương ứng ra khỏi Hàng đợi. Trước 2026-08-13 tập giá trị hợp lệ chỉ tồn tại trong `[Description]` của tool MCP và một comment cạnh property, còn handler thì `Enum.Parse<JournalEntryType>(ignoreCase: true)` thẳng — nên `entryType: "Decision"` đi qua và tắt được cảnh báo **không cần lý do**, đi vòng qua luật `Note` ≥ 20 ký tự của đường resolve. `Docs/AI-Agent-TradePlan-API.md` còn liệt kê `Decision` là giá trị hợp lệ, tức là đang mời agent làm việc đó. Nay `CreateJournalEntryCommandValidator`/`UpdateJournalEntryCommandValidator` chặn bằng **allowlist theo tên**, không phải "khác Decision là được": `Enum.Parse` nhận cả chuỗi số (`"5"` ra `Decision`) và bỏ khoảng trắng hai đầu. Vị từ đặt một chỗ dùng cho cả create lẫn update. Test có ca đối chứng quét toàn bộ enum, nên thêm loại mới mà không quyết định nó thuộc phía nào thì đỏ ngay.

**Tập giá trị hữu hạn nằm trong `inputSchema` (ADR-0015).** Agent chỉ giao tiếp qua MCP — nó không đọc `Docs/AI-Agent-TradePlan-API.md`. Mọi field có tập giá trị hữu hạn khai **enum miền thật** (SDK sinh `"enum":[…]`) kèm `[Description]` mang ngữ nghĩa: `ScenarioNodeDto.ConditionType/ActionType/Status`, `TrailingStopConfigDto.Method`, `ExitTargetDto.ActionType`, `PlanLotDto.Status`, `InvalidationRuleDto.Trigger`, và tham số phẳng `timeHorizon`/`entryMode`/`exitStrategyMode`. Field không có enum miền (`direction`, `marketCondition`, `status` của `set_trade_plan_status` — `string` thuần trên `TradePlan`) liệt kê giá trị trong `[Description]`; `[AllowedValues]` kèm theo chỉ phục vụ `completion/complete`, **không** sinh ràng buộc `enum` nên không tới được agent đọc `tools/list`. `ActionType`/`ConditionType` nullable + validator `NotNull` (quyết định không được có mặc định ngầm); `Method` nullable → `Percentage` (đơn vị đo thì được). Guard: `McpSchemaContractTests` + `McpFiniteValueSweepTests`.
...
- Giới hạn đã biết của `HasAnyValue`: với property KHÔNG nullable (`Shareholder.Percentage`, `Shareholder.Quantity`) thì 0 và "không có" là cùng một bit, nên giá trị **mặc định** được coi là "không có thông tin". Chọn phía này vì một số 0 thật bị báo "không lấy được" chỉ mất một dòng, còn một vỏ rỗng được coi là dữ liệu thì bịa ra cả một khối cổ đông.
- Số liệu **không** tính vào điều kiện chặn của cổng; panel nói rõ điều đó bằng một dòng chữ nhỏ.

**Lệch hợp đồng 24hmoney (2026-08-11):** nguồn đổi cấu trúc, 6/8 section hỏng âm thầm — mỗi hàm fetch `catch → LogWarning → return null` nên lệch hợp đồng trông y hệt "mã này không có dữ liệu". Test vẫn xanh vì fixture **tự bịa** ghim đúng cấu trúc mà nguồn đã bỏ. Nay fixture là response **thật** bắt được, để trong `tests/InvestmentApp.Infrastructure.Tests/Fixtures/Hmoney/`; bắt lại bằng `curl` khi nguồn đổi tiếp.

| Section | Đổi gì |
|---|---|
| `businessPlan` | `data` từ mảng thành object; nay là danh sách chỉ tiêu **có nhãn** kèm tiến độ (`CompanyPlanTarget`). Không còn kế hoạch cổ tức |
| `foreignTrading` | Không còn chuỗi theo ngày — chỉ còn diễn biến trong phiên + 6 mức tổng hợp. Đổi sang `ForeignTradingSummary` theo **giá trị** (tỷ VND), **không phải khối lượng** |
| `incomeStatements` | `header`/`data` → `headers`/`rows`; header là object `{year, quarter}`. `period=1` trả theo **năm**, phải dùng `period=2` mới theo quý |
| `peers` | Lồng thêm một tầng: `data.data` → `data.all.data` (chia theo sàn) |
| `company` | `/company/detail` bỏ `company_name`/`floor`, đổi sang `ownership[]`/`leadership[]`. Tên + sàn nay lấy từ `/stock/detail` |
| `dividendEvents` | Đổi hết tên khoá; ngày về dạng **epoch giây neo nửa đêm giờ VN** — format theo UTC là lùi một ngày |

Hai bẫy không nằm ở cấu trúc: tên dòng báo cáo viết hoa không nhất quán (`"LỢI NHUẬN SAU THUẾ TNDN"`) nên `Contains` phân biệt hoa thường trả null im lặng — dùng `OrdinalIgnoreCase`; và fixture chứa dữ liệu cá nhân của lãnh đạo (ngày sinh, địa chỉ) nên các trường code không đọc đã được **xoá trắng** trước khi commit.

Còn nợ: `catch → return null` vẫn khiến lệch hợp đồng không phân biệt được với "không có dữ liệu".
