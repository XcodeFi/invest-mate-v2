...
- `FundamentalsPanelComponent` (`frontend/src/app/features/company-dossier/fundamentals-panel.component.ts`) đứng cạnh ô viết trong grid `lg:grid-cols-2`. Điều kiện render mỗi khối là `hasSection()` — vừa không bị đánh dấu thiếu **vừa thật sự có payload**; chỉ tin `unavailableSections` là để một lần lệch giữa danh sách và body deref null, và một deref null làm sập cả vòng change detection nên các khối khác biến mất im lặng.
