...
| CompanyDossiers | `/api/v1/company-dossiers` | **Hồ sơ công ty (2026-08-10, ADR-0009)** — JWT. `GET` list, `GET /{symbol}`, `PUT /{symbol}` upsert (luôn `ByAgent=false`), `POST /{symbol}/confirm` (chỉ đường duy nhất đặt `ConfirmedAt`), `GET /{symbol}/gate-status` (pre-flight check trước khi tạo plan; `quantity`/`entryPrice`/`accountBalance` **bắt buộc**, thiếu → 400 — thiếu 1 trong 3 mà đoán bằng 0 sẽ chấm nhầm bậc so với `POST /trade-plans` thật). |
...
- Lệnh trade plan **đầu tiên sau khi deploy** bị chặn với mọi mã, kể cả mã đang giữ — không có grandfathering (ADR-0009 D5).
- Đường ghi trade plan của agent (ApiKey `AiAgentController` + MCP) **tắt hoàn toàn** cho tới khi chặng 2 phơi `upsert_company_dossier` — `DossierGateException` trong MCP tool không đi qua `ExceptionMiddleware` nên agent hiện chỉ nhận thông báo lỗi, mất `missing[]` (ADR-0009 D6).
