...
### Checkpoint — Slice P4 (done 2026-07-26) — HOÀN TẤT P1–P4
- **Decisions**: (1) enum nhận **chuỗi** + validate ở wrapper (`ParseEnum`) → agent nhận lỗi tiếng Việt kèm danh sách giá trị hợp lệ, chặn cả chuỗi số; (2) 3 command trả `Unit` → tool trả câu xác nhận đọc được (Unit serialize thành `{}` vô nghĩa với host); (3) guard `newStopLoss > 0` tại wrapper (domain không chặn).
- **🔒 Lỗ bảo mật phát hiện + đã vá (finding P1/90)**: `ExecuteLotCommandHandler` và `TriggerExitTargetCommandHandler` kiểm tra `plan.UserId` nhưng rồi **load và ghi vào `trade` do caller cung cấp mà không kiểm chủ sở hữu** → tradeId giả mạo ghi `TradePlanId` lên lệnh của user khác, và `review_trade_plan` sau đó cộng lệnh lạ đó vào P/L. Đã thêm transitive check (trade → portfolio → UserId), mirror đúng `LinkTradeToPlanCommandHandler`. **Ảnh hưởng cả REST**, không chỉ MCP — đây là bug sẵn có, MCP chỉ làm nó dễ khai thác hơn. Inject thêm `IPortfolioRepository` vào 2 handler + 5 test (2 test tấn công cross-user).
- **Xác nhận từ review**: `ValidationBehavior<,>` là open behavior đăng ký ở `Program.cs` → FluentValidation **có chạy** cho MCP dispatch (note ≥20 ký tự, tradePlanId cho ExecuteSell đều được enforce). `ResolveDecision` ExecuteSell có ownership 2 lớp (plan + portfolio). `SetStopLossTarget` kiểm cả `trade.PortfolioId == portfolioId`.
- **Files changed**: `Mcp/PlanActionTools.cs` (new), `tests/.../Mcp/PlanActionToolsTests.cs` (new, 12 tests), `McpToolDiscoveryTests.cs` (70 tool + 4 required-array assertion), 2 handler Application (security fix) + 2 test file mới, docs + CHANGELOG v2.70.0.
- **Tests**: Api 214, Application 245, suite **1.513** pass.
- **Behavior sẵn có đã ghi vào description (không sửa handler)**: `review_trade_plan` đòi status Executed; `execute_lot` đòi plan nhiều lô InProgress + lô Pending; `trigger_exit_target` **không idempotent** (gọi lại ghi trùng) và không có status guard; `set_stop_loss_target` không đối chiếu symbol/entryPrice với lệnh thật.
- **Affected layers**: Api + Application (security fix) + tests.
- **Next**: hết P1–P4. Còn lại: (a) PR backfill optional-param idiom cho tool cũ; (b) P5 breadth khi cần; (c) 2 follow-up của P3 (cascade sau soft-delete portfolio, `BulkTradeItem.Fee/Tax` non-nullable); (d) cân nhắc thêm status guard + idempotency cho `trigger_exit_target`/`update_stop_loss` ở tầng domain.
