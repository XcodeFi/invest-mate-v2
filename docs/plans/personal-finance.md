...
### Checkpoint — Phase 5 Frontend (PR #TBD — 2026-04-22)

- **Decisions:**
  - Giữ nguyên 1 PR (không split 5a/5b) — scope ~5 file, complexity vừa phải cho 1 review.
  - **`PersonalFinanceService`** có TS enums match BE numeric serialization (SJC=0, DOJI=1, PNJ=2, Other=99; Mieng=0, Nhan=1). Comment warning rõ để BE đổi sang `JsonStringEnumConverter` sẽ silent break.
  - **Gold form auto-calc**: pre-fetch `/gold-prices` khi user mở form (invalidate cache mỗi lần mở để không stale). User select brand+type → FE tìm matching price trong list → hiển thị "Giá Bán ra hiện tại" + "Số dư tự tính" real-time khi đổi quantity.
  - **Dashboard widget** click-through sang `/personal-finance`. Ẩn khi fetch error (log console thay vì throw UI). Có onboarding card khi `hasProfile=false`.
  - **Header nav** thêm entry "Tài chính cá nhân" dưới group "Quản lý" cạnh "Cài đặt AI".
  - **Modal form**: backdrop click close, inner `stopPropagation` để không đóng khi click vào form fields.
  - **Balance validation**: dựa BE enforcement (throw 400 → notify Vietnamese message). Acceptable cho solo user.
  - **HOTFIX backend** (Phase 4 regression phát hiện khi manual verify): `FinancialProfileRepository` CreateOne với explicit index Name đụng với existing `UserId_1` từ Phase 4 E2E test → Mongo throw `createIndexes failed: Index already exists with a different name`. Fix: bỏ explicit Name + wrap `catch (MongoCommandException) when (ex.Code is 85 or 86)` (IndexOptionsConflict/IndexKeySpecsConflict) — narrow scope, re-throw mọi exception khác.
  - **Config fix**: `appsettings.json` từ hardcoded `"https://24hmoney.vn/gia-vang"` → placeholder `"{GoldPriceProvider__PageUrl}"` theo convention dự án (URL thật chỉ trong env var). Deploy checklist mới thêm Section 11.
- **Files changed:**
  - New: `frontend/src/app/core/services/personal-finance.service.ts` (HTTP client + 6 interfaces + 2 request types + 4 static label helpers)
  - New: `frontend/src/app/features/personal-finance/personal-finance.component.ts` (~620 lines standalone: onboarding + 5-card net worth + health bar + rule checks + accounts grid + settings disclosure + account form modal với Gold auto-calc)
  - Modified: `frontend/src/app/features/dashboard/dashboard.component.ts` (thêm widget + loadNetWorth)
  - Modified: `frontend/src/app/app.routes.ts` (route `/personal-finance` với AuthGuard)
  - Modified: `frontend/src/app/shared/components/header/header.component.ts` (menu item)
  - Modified (hotfix): `src/InvestmentApp.Infrastructure/Repositories/FinancialProfileRepository.cs`
  - Modified (config convention): `src/InvestmentApp.Api/appsettings.json`
  - Modified (docs): `docs/plans/personal-finance.md` (Section 11 Deploy checklist + Phase 5 checkpoint)
- **Tests:** 0 FE .spec.ts (consistent với precedent dự án, dashboard/ai-settings cũng không có). BE 1013 tests pass no regression (Domain 658, Application 115, Infrastructure 235, Api 5).
- **Manual verify (real browser via Chrome DevTools MCP):**
  - Dashboard widget hiển thị profile cũ từ Phase 4 test (tổng 200M, health 60/100, breakdown đúng)
  - Click widget → nav sang `/personal-finance` page
  - Page render: 5-card net worth, health bar 60/100, 3 rule checks (Emergency FAIL / Investment PASS / Savings PASS), 4 account cards
  - + Thêm tài khoản → modal open, switch type sang Gold → Gold form hiển thị với dropdown + quantity input
  - Live price từ `/gold-prices` render "Giá Bán ra hiện tại: 170.000.000 đ / lượng" (SJC Miếng)
  - Nhập quantity=2 → "Số dư tự tính: **340.000.000 đ**" auto update ✓
  - Vietnamese text tất cả có dấu đầy đủ ✓
- **Surprises:**
  - Phase 4 explicit index Name (review fix) actually broke startup trên dev DB có index cũ từ E2E test. Lesson: explicit naming an existing unnamed index requires drop-and-recreate, không thể add name via CreateOne. Hotfix narrow-catch pattern mới đúng.
  - User flag hardcoded URL convention mid-phase → lưu `feedback_config_placeholder_convention.md` để future phases luôn dùng `{Section__Key}` placeholder ngay.
  - Test JWT generation via Python+PyJWT + inject localStorage qua Chrome DevTools MCP là pattern hiệu quả cho manual verify mà không cần Google OAuth flow.
- **Next (Phase 6 — Docs + CHANGELOG):**
  - Update `docs/architecture.md` (thêm `FinancialProfile` entity, `HmoneyGoldPriceProvider`, `FinancialProfileRepository`, `PersonalFinanceController`, trang `/personal-finance`).
  - Update `docs/business-domain.md` (entity map + API endpoints cho personal-finance + gold-prices).
  - Update `docs/features.md` (thêm Personal Finance vào features list + Tier 3 status → Done).
  - Update `frontend/src/assets/CHANGELOG.md` (minor version bump + entry với test counts + 24hmoney gold crawler highlight).
  - Move `docs/plans/personal-finance.md` → `docs/plans/done/`.
  - Section 10 E2E verification checklist chạy lại toàn bộ 8 items trước khi archive plan.
...
## 11. Deploy checklist (BẮT BUỘC trước khi deploy prod/staging)

> Convention dự án: `appsettings.json` không commit URL/key thật — dùng placeholder `{Section__Key}`. Env var inject giá trị lúc runtime. Section mới thêm Phase 3+4 cần setup:

- [ ] **`GoldPriceProvider__PageUrl`** env var = `https://24hmoney.vn/gia-vang` (Cloud Run Secret hoặc Cloud Build substitution). Nếu không set, provider sẽ crash vì URL là placeholder raw.
- [ ] Verify sau deploy: `curl $API/api/v1/personal-finance/gold-prices` với JWT → 8 entries trả về (nếu chỉ có error "Could not resolve..." → env var chưa set).
- [ ] `appsettings.Development.json` đã có URL thật (gitignored) — dev local không cần thêm bước.
- [ ] Reference pattern: `MarketDataProvider__BaseUrl`, `ConnectionStrings__MongoDb`, `GoogleOAuth__ClientId` — đã có sẵn trong deploy pipeline, thêm `GoldPriceProvider__PageUrl` theo cùng cách.
