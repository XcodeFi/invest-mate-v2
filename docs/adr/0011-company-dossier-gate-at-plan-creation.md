...
| D9 | **Điều kiện "request này có áp lots hay không" là MỘT biến (`willApplyLots`), tính một lần trong handler, dùng cho cả điểm bắn gate lẫn lệnh `plan.SetLots`.** Và `ResolveEffectiveGateInputs` trả thẳng `PlanSize`, không trả `(Quantity, EntryPrice)` cho caller tự nhân. | Năm cửa hậu đã phải vá trên nhánh này đều cùng một họ: *một đầu vào của phép so đọc ở thời điểm sai, hoặc từ nguồn sai*. Hai cái cuối do `SetLots` gán `Quantity = tổng lô` **sau** điểm bắn. Vá bằng cách viết lại điều kiện ở chỗ thứ hai là tạo ra cửa hậu mới, vì điều kiện của hai đường ~~không giống nhau (đường tạo có `Count > 0`, đường sửa không)~~ **từng không giống nhau** — `Lots` có phần tử + `EntryMode` bỏ trống là gate chấm theo lots còn entity giữ `Quantity` header. *(Cập nhật 2026-08-10, PR #154: hai đường giờ dùng đúng một biểu thức `EntryMode != null && Lots is { Count: > 0 }`. Sự bất đối xứng cũ không phải chủ ý — nó làm `lots: []` chạy `SetLots` rồi gán `Quantity = 0` sau điểm bắn gate, tức đúng cái họ lỗi mà D9 đang chống. **Quyết định D9 không đổi:** vẫn một biến duy nhất; giờ nó còn khớp cả hai handler nên lệch được nữa là lỗi biên dịch ở cả hai chỗ.)* Một biến thì lệch được nữa là lỗi biên dịch, hai biểu thức thì lệch âm thầm. Cùng lý do, size chấm theo **mức lớn hơn** giữa `tổng(lô × giá lô)` và `tổng lô × giá header`: mỗi vế đều bỏ trống được, chấm theo vế nhỏ hơn là mở đường hạ bậc. |
