...
**Follow-ups:**
- **Surface expansion (2026-07-23):** the same Option-A pattern was extended to 5 more domains — positions (read), watchlist (CRUD), journal-entries, journals, symbol timeline — via sibling controllers sharing a new `AiAgentControllerBase`. Same decision, no new architectural trade-off. Spec: `docs/superpowers/specs/done/2026-07-23-agent-watchlist-positions-expose-design.md`. Closed a pre-existing IDOR while exposing it: `CreateJournalCommandHandler` now asserts trade ownership via `portfolio.UserId == sub` (mirrors the #122 `CreateTrade` fix). Remaining transitive `StrategyId`/`tradeId` ownership on plan commands stays deferred to v1.1.
