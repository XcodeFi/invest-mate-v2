...
| **Nguồn ngưỡng cắt lỗ của một vị thế** | Hai nguồn, có thứ tự ưu tiên (ADR-0017): bản ghi `stop_loss_targets` **thắng khi có**, còn lại lùi về `TradePlan.StopLoss` của kế hoạch `Ready`/`InProgress`/`Executed` mới nhất cùng `PortfolioId` + `Symbol`. Loại `Draft` — kế hoạch nháp không được làm im cảnh báo cho vị thế thật đang hở. `PositionRiskItem.StopLossSource` (`"Target"`/`"Plan"`/null) + `TradePlanId` nói rõ nguồn để giao diện gửi lệnh dời SL tới đúng chỗ. Mốc điều chỉnh khi lùi về kế hoạch là `TradePlan.PricesSetAt`, **không** phải `UpdatedAt`. |
...
19. **`MaxDrawdownAlertPercent` mang HAI nghĩa (2026-08-12, ADR-0014)**: risk-dashboard đọc nó theo nghĩa gốc — "báo khi danh mục xuống N% từ đỉnh". Trần khối lượng theo ngân sách biến động đọc nó là **mức lỗ ở độ tin cậy 95% trong 21 phiên**, quy về biến động năm bằng `MaxDD / (1,645 × √(21/252))` — mặc định 10% ⇒ ngân sách 21,1%/năm. Hệ số 1,645 là cùng hằng số `CalculateValueAtRiskAsync` đang dùng nên hai chỗ nhất quán. Hai cách đọc cùng hướng (đặt thấp = thận trọng ở cả hai nơi) nhưng **không suy ra được từ tên trường** — đổi ý nghĩa một chỗ là lệch chỗ kia. Chân trời 21 phiên là quyết định có đánh đổi: diễn giải theo năm cho ngân sách 6,1%/năm trong khi danh mục 7 mã thật đo được 19,4%/năm, tức trần **luôn** bằng 0 và tính năng chết ngay ngày đầu.
20. **Trần khối lượng là cảnh báo, không phải cổng (2026-08-12, ADR-0014)**: vượt trần vẫn lưu được kế hoạch. Nguyên lý kế thừa ADR-0012 — *nguồn dữ liệu quyết định quyền chặn*: cổng hồ sơ chặn được vì đọc thứ người dùng tự viết; tỷ trọng ngành không chặn vì đọc nhãn provider ngoài; trần khối lượng đọc **ước lượng thống kê từ ~65 quan sát**, yếu hơn cả nhãn ngành. Chỉ áp cho lệnh **MUA** — phép chiếu cộng quy mô lệnh vào giá trị danh mục nên với lệnh BÁN nó báo rủi ro tăng đúng lúc lệnh đó làm giảm.
