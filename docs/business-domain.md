...
| Decisions | `/api/v1/decisions/queue` | **Decision Queue (PR-2 P3, 2026-05-04)** — GET aggregate 3 nguồn alert thành 1 list duy nhất: (1) StopLossHit từ `IRiskCalculationService` filter `DistanceToStopLossPercent ≤ 2%` (≤ 0 = Critical, ≤ 2 = Warning), (2) ScenarioTrigger từ `IScenarioAdvisoryService` (Warning), (3) ThesisReviewDue từ `GetPendingThesisReviewsQuery` (DaysOverdue ≥ 3 = Critical, else Warning). Dedupe theo (Symbol, PortfolioId) giữ severity cao nhất, ưu tiên StopLossHit. Sort severity desc → DueAt asc. Response `DecisionQueueDto { Items, TotalCount }`. Item Id composite `{type}:{sourceId}`. |
...
| MCP (ApiKey scheme) | `/mcp` | **Model Context Protocol server (2026-07-24)** — cùng toàn bộ bề mặt agent trên nhưng dạng **tool có schema** cho MCP client (Claude Desktop/IDE/NPU). Streamable HTTP, stateless, sau ApiKey scheme (`UserId` = `sub`). **70 tool** (13 lớp `[McpServerToolType]` trong `Mcp/`) re-dispatch đúng MediatR command/query — không thêm business logic. 29 tool mirror bề mặt `AiAgent*Controller`; **8 tool P0 Decision & Risk (chỉ đọc, 2026-07-25)** mở query chưa có ở REST agent: decision queue (gộp alert SL + scenario + thesis review), discipline score/streak, portfolio risk (VaR/Sharpe/drawdown), stop-loss targets, trailing-stop alerts, pending thesis reviews, scenario advisories. **+1 tool `get_daily_digest` (2026-07-26)** — bản tin hằng ngày (danh mục + số dư + sizing) dạng MCP tool, thay cho REST `POST /ai/daily-digest` phía agent. **8 tool P1 Analytics (chỉ đọc, 2026-07-26)** — "tôi đang làm ăn thế nào": performance (total/MTD/YTD), equity curve, monthly returns, savings comparison (alpha vs gửi tiết kiệm, param `annualRate`/`asOf`), campaign analytics (win rate, lọc `timeHorizon`), net worth summary, flow history (nạp/rút/cổ tức, lọc `from`/`to`), adjusted return (TWR/MWR) — 6/8 per-portfolio, ownership check ở handler. **8 tool P2 Market Research (chỉ đọc, 2026-07-26)** — "nghiên cứu giúp tôi mã này": stock detail, giá hiện tại, lịch sử giá (mặc định 3 tháng), phân tích kỹ thuật (MA/RSI/MACD/Bollinger), tìm mã, tổng quan thị trường, top biến động (theo sàn), giá hàng loạt. Dữ liệu **công khai** → query không mang `UserId`, tool không inject `IHttpContextAccessor`; symbol chuẩn hóa uppercase tại wrapper. **8 tool P3 Portfolio & Trade Management (2026-07-26)** — đọc: `get_portfolio` (chi tiết 1 danh mục), `get_trades_by_portfolio` (phân trang + lọc mã/loại lệnh); ghi (`Destructive`): tạo/sửa/xóa danh mục, xóa lệnh, gắn lệnh vào kế hoạch, ghi lệnh hàng loạt (`bulk_create_trades` — kết quả có thể thành công một phần, fee/tax phải truyền sẵn khác `create_trade`). **8 tool P4 Plan Execution (2026-07-26, toàn bộ `Destructive`)** — agent hành động theo kế hoạch: `resolve_decision` (ExecuteSell = **bán thật**, HoldWithJournal = ghi nhật ký lý do giữ), `review_trade_plan`, `abort_trade_plan` (thesis bị phá), `execute_lot`, `update_stop_loss`, `trigger_exit_target`, `set_stop_loss_target`, `set_risk_profile`. Enum nhận dạng chuỗi + validate ở wrapper (báo lỗi tiếng Việt kèm danh sách giá trị hợp lệ). Read → `ReadOnly`, write → `Destructive` (host tự hỏi xác nhận). Additive: REST `/ai/agent/*` giữ nguyên; MCP thay `/doc` markdown bằng `tools/list`. |
