...
| MCP (ApiKey scheme) | `/mcp` | **Model Context Protocol server (2026-07-24)** — cùng toàn bộ bề mặt agent trên nhưng dạng **tool có schema** cho MCP client (Claude Desktop/IDE/NPU). Streamable HTTP, stateless, sau ApiKey scheme (`UserId` = `sub`). **29 tool** (7 lớp `[McpServerToolType]` trong `Mcp/`) re-dispatch đúng MediatR command/query của các `AiAgent*Controller` — không thêm business logic. Read → `ReadOnly`, write → `Destructive` (host tự hỏi xác nhận). Additive: REST `/ai/agent/*` giữ nguyên; MCP thay `/doc` markdown bằng `tools/list`. |
