...
- **`Trade.TradeDate` lưu hai hình dạng khác nhau, và một trong hai lệch một ngày** — property không có `[BsonDateTimeOptions(Kind = DateTimeKind.Utc)]`, nên driver coi `DateTime` `Unspecified` là giờ local rồi quy về UTC: lệnh gửi `TradeDate: "2026-08-13"` được lưu thành `2026-08-12T17:00:00Z`. Lệnh **không** truyền ngày lại lấy `DateTime.UtcNow` (`Kind = Utc`) nên lưu đúng mốc thật. Hai hình dạng cùng tồn tại trong một collection. Frontend không thấy gì vì browser quy mốc UTC về +07 rồi mới render. Chỗ đau là mọi phép đọc `.Date` **phía server**: `PositionBuilder.cs:52` (`t.TradeDate.Date <= asOfDate`, và thứ tự so với sự kiện quyền cùng ngày), `AiAssistantService.cs:2059` (cửa sổ lệnh gần đây), `BehavioralAnalysisService.cs:167` (`GroupBy(t => t.TradeDate.Date)` — hai lệnh cùng một ngày thật rơi vào hai nhóm nếu khác hình dạng lưu), và `AiAssistantService.cs:245` format `dd/MM/yyyy` trực tiếp trên mốc UTC → in sớm một ngày trong digest gửi cho AI. `SettlementCalculator` đi qua `VietnamDate.DayOf()` nên đúng cho **cả hai** hình dạng; các chỗ còn lại thì chưa. Sửa gốc (thêm attribute + migration rebase dữ liệu cũ) phải đi kèm ADR riêng: thêm attribute mà không rebase là đổi nghĩa toàn bộ bản ghi lịch sử trong im lặng.
