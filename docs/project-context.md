...
- **Tập giá trị hợp lệ chỉ nằm trong câu mô tả thì không phải là luật** — `create_journal_entry` khai `entryType` là `string` thuần với `[Description]` liệt kê 5 loại, không có validator, và handler `Enum.Parse<JournalEntryType>(ignoreCase: true)` thẳng. Nên `entryType: "Decision"` đi qua — mà loại đó là **cờ dập cảnh báo**: `GetDecisionQueueQuery` lọc mọi thẻ có mục `Decision` cùng ngày ra khỏi Hàng đợi quyết định. Kết quả: agent tắt được cảnh báo **không cần lý do**, đi vòng qua đúng luật `Note` ≥ 20 ký tự mà đường resolve dựng ra để bắt người dùng nghĩ. Tệ hơn, `Docs/AI-Agent-TradePlan-API.md` còn **liệt kê `Decision` là giá trị hợp lệ** — tài liệu không chỉ thiếu chặn, nó đang mời. Hai bài học: (a) mọi field có tập giá trị hữu hạn phải có allowlist ở validator, `[Description]` là tài liệu chứ không phải guard (ADR-0015 nói đúng chuyện này nhưng chỉ phủ field có enum miền — `string` thuần lọt lưới); (b) allowlist phải khớp **theo tên**, không phải "parse được thì cho qua": `Enum.Parse` nhận cả chuỗi số (`"5"` ra giá trị thứ 6) và bỏ khoảng trắng hai đầu, nên lọc kiểu "khác Decision là được" vẫn bị lách. Test phải có ca đối chứng quét toàn bộ enum để loại mới thêm sau không lặng lẽ rơi về phía sai.

- **Hành động hoàn tác được nhưng cờ dập cảnh báo thì không** — nút BÁN trên Hàng đợi quyết định POST `ExecuteSell`, và handler ghi **hai** thứ: lệnh bán, và một `JournalEntry` loại `Decision` làm cờ để `LoadResolvedTodayAsync` lọc thẻ đó ra trong hết ngày VN. `DeleteTradeCommandHandler` xoá lệnh + ghi audit, **không chạm cờ**. Nên xoá lệnh bán ở trang giao dịch thì vị thế và stop-loss quay về đúng như trước, chỉ riêng cảnh báo là mất tới 00:00 — người dùng đọc thành "hệ thống ăn mất cảnh báo của tôi". Cờ còn khoá theo `TradePlanId` nên nó dập luôn cả `ScenarioTrigger`/`ThesisReviewDue` của cùng kế hoạch. Hình dạng chung: **một hành động ghi N thứ thì đường hoàn tác phải biết cả N**, không thì mỗi thứ bị bỏ sót là một trạng thái không quay về được. Cách chữa gọn nhất ở đây không phải nối thêm cascade xoá, mà là **không ghi gì lúc bấm** — điều hướng sang màn hình bán để chính việc tạo lệnh mới là thời điểm có gì đó xảy ra (đổi 2026-08-13).
...
- **Một biến môi trường thêm vì LOG đã lặng lẽ đổi ngữ nghĩa lưu ngày** — `Trade.TradeDate` không có `[BsonDateTimeOptions(Kind = DateTimeKind.Utc)]`, nên với `DateTime` `Kind = Unspecified` driver quy đổi theo **giờ local của tiến trình**. Giờ local đó là một biến triển khai: `cloudbuild.yaml` đặt `TZ=Asia/Ho_Chi_Minh` từ PR #163 (thêm vào để Serilog in log theo giờ VN), trước đó container chạy UTC. Nên cùng một payload cho ra hai kết quả khác nhau tuỳ ngày deploy, và **ba** hình dạng cùng tồn tại trong `trades` (đếm trên prod 2026-08-13):

  | Hình dạng | Cách sinh | Ví dụ đã lưu | Ngày lịch VN đúng |
  |---|---|---|---|
  | Mốc thật | không truyền ngày → `DateTime.UtcNow` | `TradeDate == CreatedAt` | `+7h` rồi `.Date` |
  | Ngày-trần bị dịch | ngày-trần gửi tới tiến trình chạy +07 | `17:00:00Z` hôm trước | `+7h` rồi `.Date` |
  | Giờ treo tường đóng dấu UTC | `datetime-local` gửi tới tiến trình chạy UTC (18 dòng, tháng 3–4/2026) | `TradeDate == CreatedAt + 7h` | `.Date` trần, **không** cộng offset |

  `VietnamDate.DayOf()` (+7h) đúng cho hai hình dạng đầu — tức đúng cho mọi bản ghi tạo từ PR #163 trở đi — nhưng **vượt một ngày** với hình dạng thứ ba khi giờ lưu ≥ 17:00Z. 18 dòng đó đều ở tháng 3–4/2026, nằm ngoài cửa sổ thanh toán rất xa nên không chạm vào con số "chờ về"; chỗ chúng còn cắn là `PositionBuilder.cs:52` (`t.TradeDate.Date <= asOfDate`, và thứ tự so với sự kiện quyền cùng ngày), `AiAssistantService.cs:2059` (cửa sổ lệnh gần đây), `BehavioralAnalysisService.cs:167` (`GroupBy(t => t.TradeDate.Date)`), và `AiAssistantService.cs:245` format `dd/MM/yyyy` trực tiếp trên mốc UTC. Frontend không chịu ảnh hưởng vì browser quy về +07 trước khi render. **Đừng rải `DayOf()` sang các chỗ đó trước khi rebase 18 dòng cũ** — sẽ đúng chỗ mới mà làm sai chỗ cũ. Sửa gốc cần ADR + migration riêng; thêm attribute mà không rebase là đổi nghĩa toàn bộ bản ghi lịch sử trong im lặng. Bài học chung: khi ngữ nghĩa lưu trữ phụ thuộc giờ local của tiến trình thì **cấu hình hạ tầng trở thành một phần của lược đồ dữ liệu** — đọc `.Date` trần trên một `DateTime` lấy từ DB luôn phải hỏi "mốc này được ai đóng dấu, ở múi giờ nào".
