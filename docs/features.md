...
| `/company-dossier/:symbol` | `CompanyDossierDetailComponent` | Chi tiết hồ sơ, **hai chế độ**: mở ra là bản đọc (`DossierViewComponent`), bấm Sửa mới ra form. Kèm **Sao chép cho AI** (trợ lý không nối MCP), **Chia sẻ** (gửi cho tài khoản khác qua clipboard) và **Dán nội dung** (nhận cả hai nguồn) |
...
- **P4 — Inline action buttons:** mỗi DecisionItem có 2 button resolve in-place. **Qua MCP:** agent đọc hàng đợi bằng `get_decision_queue` và xử lý theo hướng GIỮ bằng `hold_decision` (lý do ≥ 20 ký tự, cùng validator với giao diện). **Không có đường BÁN qua MCP** — `action` không phải tham số của tool nên `ExecuteSell` không tới được; agent muốn bán thì dùng `create_trade` sau khi thống nhất khối lượng với người dùng. `JournalEntryType.Decision` cũng không đặt được từ `create_journal_entry`/`update_journal_entry` (400) vì đó là cờ dập cảnh báo.
- `🔪 BÁN THEO KẾ HOẠCH` chỉ enable khi item có `tradePlanId` — **điều hướng sang màn hình bán `/trades/create` với form điền sẵn** (`symbol`, `portfolioId`, `direction=Sell`, `planId`, `price`, `quantity` từ kế hoạch), người dùng sửa được số lượng rồi mới ghi lệnh. Không tạo lệnh và không resolve lúc bấm (đổi 2026-08-13). Đường `ExecuteSell` ở backend vẫn còn nhưng không còn caller từ FE. `✋ GIỮ + GHI LÝ DO` expand inline note textarea (≥ 20 ký tự sau Trim → enable submit), tạo `JournalEntry` với `EntryType=Decision` + Tags `["decision-hold", "trigger:{type}"]`. Optimistic remove khỏi list sau success. Per-item error map hiện error ở cả BÁN lẫn GIỮ flow. Defense-in-depth: validate cả `plan.UserId` và `portfolio.UserId` match request.UserId.
