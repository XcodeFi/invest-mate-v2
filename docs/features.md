...
- **Ngoại lệ — dời SL (ADR-0017):** plan `InProgress`/`Executed` dời được SL qua nút "Dời SL" (ngoài form, đi `PATCH /{id}/stop-loss`). Nới SL bắt buộc lý do + đếm vào điểm kỷ luật. `Executed` là trạng thái **đang giữ vị thế**, không phải đã đóng — `Reviewed` mới là đã đóng.
